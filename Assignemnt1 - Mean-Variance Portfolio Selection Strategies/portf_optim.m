...
strategy_functions = {'strat_buy_and_hold' 'strat_equally_weighted' 'strat_min_variance' 'strat_max_Sharpe' 'strat_max_Sharpe_V1'};
strategy_names     = {'Buy and Hold' 'Equally Weighted Portfolio' 'Mininum Variance Portfolio' 'Maximum Sharpe Ratio Portfolio' 'Maximum Sharpe Ratio Portfolio V1'};
...
% introducing new variable to help analyze Q3
cash_reserve = 0;
...
for (period = 1:N_periods)
   ...
   for(strategy = 1:N_strat)
      ...
      if(period==1) %initiation
         ...
         % for varivation of max-sharpe strategy,
         if(strategy == 5) 
            %take 90% to invest
            curr_positions = floor(curr_positions*0.9);
            %leave 10% as reserved cash at the beginning of period 1.
            cash_reserve = init_value*0.1; 
         end
         portf_value{strategy} = zeros(N_days,1);
      else
         cash_reserve = reserve{strategy,period-1};
         curr_positions = x{strategy,period-1};
         curr_cash = cash{strategy,period-1};
      end

      % Compute strategy
      [x{strategy,period} cash{strategy,period} weight{strategy,period} reserve{strategy,period}] = fh_array{strategy}(curr_positions, curr_cash, mu, Q, cur_prices, cash_reserve);
...
figure(1);
plot(strat_result_1,"r")
hold on
plot(strat_result_2,"k")
hold on
plot(strat_result_3,"b")
hold on
...
% figure 2 dynamic change for strategy 3
x_3 = [weight{3,:}];
x_3 = [w_init x_3];
x_4 = [weight{4,:}];
x_4 = [w_init x_4];
...
% figure 3 dynamic change for strategy 4
figure(3);
plot([0:12],x_4');
legend('Stock 1','Stock 2', 'Stock 3', 'Stock 4','Stock 5','Stock 6','Stock 7','Stock 8','Stock 9','Stock 10','Stock 11','Stock 12','Stock 13','Stock 14','Stock 15','Stock 16','Stock 17','Stock 18','Stock 19','Stock 20');
xlabel('Period'); 
