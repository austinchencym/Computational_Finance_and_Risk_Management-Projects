function  [x_optimal cash_optimal weight] = strat_max_Sharpe(x_init, cash_init, mu, Q, cur_prices)
...
%     trans_asset=portfolio_value*0.01;
    ...
    lb=zeros(21,1);
    ub=inf*ones(21,1);
    A1=[mu-r_rf/252, ones(20,1)];% first constraint and second constraint coefficient for y
...
% if no solution can be obtained during financial crisis, keep the portfolio unchanged
  if (strcmp(cplex2.Solution.statusstring,'infeasible') == 1)
        x_optimal = x_init;
        cash_optimal = cash_init;
        weight = cur_prices' .* x_init / portfolio_value;
  else
